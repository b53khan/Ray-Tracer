...
bool GeometryNode::hit(Ray &ray, float t_min, float t_max, Intersection &record) {
	Ray newRay(vec3(invtrans * vec4(ray.origin, 1.0)), vec3(invtrans * vec4(ray.direction, 0.0)));
	Intersection tmp_record;
	float closest = t_max;
	tmp_record.material = nullptr;
	bool result = false;

	if (m_primitive->hit(newRay, t_min, t_max, tmp_record)) {
